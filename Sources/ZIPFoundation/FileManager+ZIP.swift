    ...
    public func zipItem(at sourceURL: [URL], to destinationURL: URL,
        ...
        DispatchQueue.global().async {
            do {
                ...
                guard fileManager.itemExists(at: sourceURL[0]) else {
                    throw CocoaError(.fileReadNoSuchFile, userInfo: [NSFilePathErrorKey: sourceURL[0].path])
                }
                ...
                let isDirectory = try FileManager.typeForItem(at: sourceURL[0]) == .directory
                if isDirectory {
                    var subPaths = try self.subpathsOfDirectory(atPath: sourceURL[0].path)
                    ...
                    if let progress = progress {
                        totalUnitCount = subPaths.reduce(Int64(0), {
                            let itemURL = sourceURL[0].appendingPathComponent($1)
                    ...
                    let directoryPrefix = sourceURL[0].lastPathComponent
                    for entryPath in subPaths {
                        let finalEntryPath = shouldKeepParent ? directoryPrefix + "/" + entryPath : entryPath
                        let finalBaseURL = shouldKeepParent ? sourceURL[0].deletingLastPathComponent() : sourceURL[0]
                        if let progress = progress {
                            let itemURL = sourceURL[0].appendingPathComponent(entryPath)
                    ...
                    progress?.totalUnitCount = archive.totalUnitCountForAddingItem(at: sourceURL[0])
                    let baseURL = sourceURL[0].deletingLastPathComponent()
                    try archive.addEntry(with: sourceURL[0].lastPathComponent, relativeTo: baseURL,
                                         compressionMethod: compressionMethod, progress: progress)
                }
                
                if sourceURL.count >= 2 {
                    guard let archive = Archive(url: destinationURL, accessMode: .update) else {
                        return
                    }
                    
                    for i in 1 ..< sourceURL.count {
                        try archive.addEntry(with: sourceURL[i].lastPathComponent, relativeTo: sourceURL[i].deletingLastPathComponent())
                    }
                }
    ...
    public func customUnzipItem(at sourceURL: URL, to destinationURL: URL, skipCRC32: Bool = false,
        ...
        DispatchQueue.global().async {
            do {
                ...
                let entries: [Entry] = self.deletingMeta(archive: archive, preferredEncoding: preferredEncoding)
                ...
                if entries.count >= 2 {
                    // 2번 유형의 압축파일인지 검사
                    if self.isOneFolderDown(entries: entries, preferredEncoding: preferredEncoding) {
                        // 엔트리 경로를 다 바꿔줘야됨, 폴더생성아님
                        oneFolderDownName = self.getFirstName(entries, preferredEncoding: preferredEncoding)
                        if fileManager.fileExists(atPath: destinationURL.path + "/\(oneFolderDownName)") {
                            oneFolderDownName = self.getSaveLocationName(path: destinationURL.path + "/\(oneFolderDownName)")
                        }
                    } else {
                        //중복검사
                        ...
                        if fileManager.fileExists(atPath: sourceURL.deletingPathExtension().path) {
                            folderName = self.getSaveLocationName(path: sourceURL.path)
                        }
                        newSourceURL = self.appendingURL(origin: newSourceURL, component: folderName)
                        do {
                ...
                for entry in entries {
                    ...
                    if entries.count >= 2 {
                        if self.isOneFolderDown(entries: entries, preferredEncoding: preferredEncoding) {
                            path = self.changeEntryPath(entry: entry, folderName: oneFolderDownName, preferredEncoding: preferredEncoding)
                            entryURL = self.appendingURL(origin: entryURL, component: path)
                            createdList.append(entryURL)
                        } else {
                            entryURL = self.appendingURL(origin: newSourceURL, component: path)
                        }
                    } else { // 원소가 하나일때
                        let fileExtension = self.getUrl(path: path).pathExtension
                        let fileURL = self.appendingURL(origin: destinationURL, component: path)
                        if fileManager.fileExists(atPath: fileURL.path) {
                            let newFileName = self.getSaveLocationName(path: fileURL.path)
                            entryURL = self.appendingURL(origin: entryURL, component: newFileName).appendingPathExtension(fileExtension)
                        } else {
                            entryURL = fileURL
                        }
                    }
